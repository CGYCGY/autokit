...
AI_DOCS: `AI_DOCS/`
APP_DOCS: `APP_DOCS/`
BROWSER: the OS default browser — pick the first opener that exists (`command -v`): `wslview` (WSL) → `xdg-open` (Linux) → `open` (macOS) → `cmd.exe /c start ""` (Windows). On WSL, if only Windows openers are available, convert the path with `wslpath -w` first.
...
<main>
  ...
  <section id="questionables">
    ...
    <details>
      <summary>{{QUESTION_TITLE}}</summary>
